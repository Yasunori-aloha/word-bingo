 ...
 */
const judgeBingo = (bingoSize, wordCells) => {
   ...
   */
  const judgeVerticalAndHorizontal = (wordCellIsOpen) => {
   ...
   * @param wordCellIsOpen 単語マスが空いているかどうか
   */
  const judgeDiagonal = (wordCellIsOpen) => {
    for (let i = 0; i < bingoSize; i++) {
      if (!wordCellIsOpen(i)) {
        return false;
      }
